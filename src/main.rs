...
//            bindless_context: Some(&Default::default()),
    ...
    let (host_a_f16, host_a_f32);
    if use_fp16 {
        host_a_f16 = Some(Buffer::from_iter(
            &mem_alloc,
            &BufferCreateInfo {
        ...
        host_a_f32 = Some(Buffer::from_iter(
            &mem_alloc,
            &BufferCreateInfo {
                usage: BufferUsage::TRANSFER_SRC,
                ..Default::default()
    ...
    let (host_b_f16, host_b_f32);
    if use_fp16 {
        host_b_f16 = Some(Buffer::from_iter(
            &mem_alloc,
            &BufferCreateInfo {
                usage: BufferUsage::TRANSFER_SRC,
                ..Default::default()
        ...
        host_b_f32 = Some(Buffer::from_iter(
            &mem_alloc,
            &BufferCreateInfo {
                usage: BufferUsage::TRANSFER_SRC,
                ..Default::default()
    ...
    if use_fp16 {
        ...
        cb.copy_buffer(CopyBufferInfoTyped::new(host_a_f16.as_ref().unwrap().clone(), a_slice.clone())).unwrap();
    } else {
        let a_slice: Subbuffer<[f32]> = Subbuffer::from(resources.buffer(a_id).unwrap().buffer().clone()).reinterpret();
        cb.copy_buffer(CopyBufferInfoTyped::new(host_a_f32.as_ref().unwrap().clone(), a_slice.clone())).unwrap();
    }
    ...
    if use_fp16 {
        ...
        cb.copy_buffer(CopyBufferInfoTyped::new(host_b_f16.as_ref().unwrap().clone(), b_slice.clone())).unwrap();
    } else {
        let b_slice: Subbuffer<[f32]> = Subbuffer::from(resources.buffer(b_id).unwrap().buffer().clone()).reinterpret();
        cb.copy_buffer(CopyBufferInfoTyped::new(host_b_f32.as_ref().unwrap().clone(), b_slice.clone())).unwrap();
    }
    let cb = cb.build().unwrap();
    let future = sync::now(device.clone())            // start with an idle future
        .then_execute(queue.clone(), cb).unwrap()             // submit to the queue
